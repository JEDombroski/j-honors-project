...
// Create a custom collision function
function collide(nodes) {
    let radius = 30; // Define minimum distance to maintain
    return function(d) {
        nodes.forEach(function(d2) {
            if (d === d2) return; // Don't check for collision with itself
            const dx = d.x - d2.x;
            const dy = d.y - d2.y;
            const distance = Math.sqrt(dx * dx + dy * dy);
            const minDistance = radius * 2; // We double the minimum radius to avoid overlap

            if (distance < minDistance) {
                const angle = Math.atan2(dy, dx);
                const targetX = d.x + Math.cos(angle) * (minDistance - distance);
                const targetY = d.y + Math.sin(angle) * (minDistance - distance);
...
// Create simulation with improved repulsion and custom collision
const simulation = d3.forceSimulation(nodes)
    .force("charge", d3.forceManyBody().strength(-150)) // Increase repulsion force
    .force("center", d3.forceCenter(width / 2, height / 3)) // Keeps nodes centered
    .force("collide", collide(nodes))  // Apply custom collision function
