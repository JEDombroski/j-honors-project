    ...
    node.on("click", (event, d) => {
        if (d.type === "theme") {
            ...
            if (themeSelected === clickedTheme) {
                ...
                // Select theme and move event nodes
                ...
                svg.selectAll("circle")
                    ...
                    .duration(500)
                    .attr("cx", d => {
                        if (d.type === "theme" && d.id === clickedTheme) {
                            return width / 2; // Center the theme node
                        }
                        if (relatedNodes.has(d.id)) {
                            return width / 2; // Move related events
                        }
                        return d.x; // Keep non-related nodes in place
                    })
                    .attr("cy", d => {
                        if (d.type === "theme" && d.id === clickedTheme) {
                            return height / 2; // Center the theme node
                        }
                        if (relatedNodes.has(d.id)) {
                            return height / 2; // Move related events
                        }
                        return d.y; // Keep non-related nodes in place
                    })
                    .attr("opacity", d => {
                        if (relatedNodes.has(d.id)) {
                            return 1; // Full opacity for related events
                        }
                        return 0.1; // Fade non-related nodes
                    });
