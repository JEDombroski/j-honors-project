...
const tooltip = d3.select("body")
    ...
    .on("mouseover", (event, d) => {
        if (d.type === "item") {  // Only show tooltip for event nodes (items)
        console.log('Mouse over node', d);
            tooltip.style("visibility", "visible")
                .html(`<strong>Name:</strong> ${d.id} <br> <strong>Date:</strong> ${d.date}`)  // Adjust this based on your data structure
                .style("top", (event.pageY + 10) + "px")  // Position tooltip slightly below the mouse cursor
                .style("left", (event.pageX + 10) + "px");  // Position tooltip slightly to the right of the mouse cursor
        }
    })
...
// Add click event to event nodes to show the modal
node.on("click", (event, d) => {
    if (d.type === "item") {
