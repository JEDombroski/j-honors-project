...
d3.json("data.json").then(data => {
    ...
    const simulation = d3.forceSimulation(nodes)
        .force("link", d3.forceLink(links).id(d => d.id).distance(100)) // This is kept, but no lines will be drawn
        .force("charge", d3.forceManyBody().strength(-300))
        .force("center", d3.forceCenter(width / 2, height / 2));

    // Remove links (lines)
    // We don’t need to add line elements since we're no longer using them
