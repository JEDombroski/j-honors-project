...
function focusOnTheme(themeNode) {
    simulation.force("x", d3.forceX(d => {
        return d.type === "item" ? themeNode.x + 100 * Math.cos(d.index * (2 * Math.PI / 6)) : width / 2;
    }).strength(d => d.type === "item" ? 0.5 : 0.05));

    simulation.force("y", d3.forceY(d => {
        return d.type === "item" ? themeNode.y + 100 * Math.sin(d.index * (2 * Math.PI / 6)) : height / 2;
    }).strength(d => d.type === "item" ? 0.5 : 0.05));
