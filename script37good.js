    ...
    .on("mouseover", (event, d) => {
        if (d.type === "item") {  // Only show tooltip for item nodes
            tooltip.style("visibility", "visible")
                .html(`<strong>Name:</strong> ${d.id} <br> <strong>Date:</strong> ${d.date ? d.date: "Unknown"}`)
...
function showModal(eventData) {
    ...
    document.getElementById("modalDate").innerText = eventData.date ? eventData.date : "Date Unknown";
    document.getElementById("modalDescription").innerText = eventData.description;
    document.getElementById("modalDescription").style.fontWeight="normal";
...
window.onclick = function(event) {
 ...
 const images = document.querySelectorAll('#modalPhotos img');
        images.forEach(img => {
            img.addEventListener('click', () => {
                img.classList.toggle('expanded'); // Toggle the expanded class on image click
            });
        });
