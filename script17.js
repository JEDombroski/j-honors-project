...
// Create simulation for node positioning
// Create simulation for node positioning
// Create simulation with larger collision radius
const simulation = d3.forceSimulation(nodes)
    .force("charge", d3.forceManyBody().strength(-100))  // Increase repulsion force
    .force("center", d3.forceCenter(width / 2, height / 3))
    .force("collide", d3.forceCollide(40))  // Larger collision radius
    .force("x", d3.forceX().strength(0.1).x(d => Math.max(20, Math.min(width - 20, d.x))))
    .force("y", d3.forceY().strength(0.1).y(d => Math.max(20, Math.min(height - 20, d.y))))
    .on("tick", () => {
