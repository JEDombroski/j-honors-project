...
function focusOnTheme(themeNode) {
    // Get only the event nodes linked to this theme
    const relatedEventNodes = links
        .filter(link => link.target === themeNode.id)
        .map(link => link.source);

    // Update force for only related event nodes
    simulation.force("x", d3.forceX(d => {
        return relatedEventNodes.includes(d.id) 
            ? themeNode.x + 100 * Math.cos(d.index * (2 * Math.PI / relatedEventNodes.length)) 
            : d.x;  // Unrelated nodes stay where they are
    }).strength(d => relatedEventNodes.includes(d.id) ? 0.5 : 0));

    simulation.force("y", d3.forceY(d => {
        return relatedEventNodes.includes(d.id) 
            ? themeNode.y + 100 * Math.sin(d.index * (2 * Math.PI / relatedEventNodes.length)) 
            : d.y;
    }).strength(d => relatedEventNodes.includes(d.id) ? 0.5 : 0));
