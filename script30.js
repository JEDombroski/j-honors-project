...
function showModal(eventData) {
    // Set modal content
    document.getElementById("modalTitle").innerText = eventData.id;  // Title of the event
    document.getElementById("modalDate").innerText = eventData.date;  // Date of the event
    document.getElementById("modalDescription").innerText = eventData.description;  // Event description

    // Check if there are photos (assuming 'photoFilenames' or 'photos' field exists in the data)
    if (eventData.photoFilenames && eventData.photoFilenames.length > 0) {
        console.log("photo exists");
        let photosHtml = eventData.photoFilenames.map(filename => {
            let imagePath = `images/${filename}`; // Adjust the path to your image directory
            console.log("Looking for image:", imagePath);
            return `<img src="${imagePath}" alt="${filename}" style="max-width: 200px; display: block; margin: 5px 0;">`;
        }).join("");
        
        // Add photos to the modal content
        document.getElementById("modalPhotos").innerHTML = photosHtml; // Assuming you have an element with id 'modalPhotos'
    } else {
        // If no photos are found, clear the photos section in the modal
        document.getElementById("modalPhotos").innerHTML = "No photos available for this event.";
    }
